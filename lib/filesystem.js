...
class Filesystem {
  ...
  getNode (p) {
    ...
    let node = this.searchNodeFromDirectory(dirname)

    if (!node && os.platform() === 'win32') {
      // we need to fix the slashes if the asar was created on mac and is being extracted on windows
      node = this.searchNodeFromDirectory(dirname.replace(/\//g, '\\'))
    }
