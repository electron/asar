...
class Filesystem {
  ...
  insertFile (p, shouldUnpack, file, options, callback) {
    ...
    if (options.transform instanceof Array && options.transform.length) {
      options.transform = options.transform.filter(() => true)

      const doTransform = (path, transformers, idx) => {
        const transform = transformers.hasOwnProperty(idx) ? transformers[idx](p) : false
        ...
        if (transform) {
          ...
          writeStream.on('close', () => {
            file.transformed = {
              path: tmpFile.name,
              stat: fs.lstatSync(tmpFile.name)
            }
            doTransform(tmpFile.name, transformers, ++idx)
          })
        } else {
          return handler()
        }
      ...
      return doTransform(p, options.transform, 0)
    } else {
