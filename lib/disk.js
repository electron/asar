...
module.exports.readArchiveHeader = async function (archive) {
  const fd = await fsOpen(archive, 'r')
  let size, headerBuf, res
  try {
    const sizeBuf = Buffer.alloc(8)
    res = await fsRead(fd, sizeBuf, 0, 8, null)
    if (res.bytesRead !== 8) {
    ...
    res = await fsRead(fd, headerBuf, 0, size, null)
...
module.exports.readFilesystem = async function (archive) {
  if (!filesystemCache[archive]) {
    const header = await this.readArchiveHeader(archive)
...
module.exports.readFile = async function (filesystem, filename, info) {
  let buffer = Buffer.alloc(info.size)
  if (info.size <= 0) { return buffer }
  if (info.unpacked) {
    // it's an unpacked file, copy it.
    buffer = await fs.readFile(path.join(`${filesystem.src}.unpacked`, filename))
  } else {
    // Node throws an exception when reading 0 bytes into a 0-size buffer,
    // so we short-circuit the read in this case.
    const fd = await fsOpen(filesystem.src, 'r')
    try {
      const offset = 8 + filesystem.headerSize + parseInt(info.offset)
      await fsRead(fd, buffer, 0, info.size, offset)
    } finally {
      await fsClose(fd)
    }
  }
