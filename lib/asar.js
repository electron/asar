...
module.exports.extractFileAsync = async function (archive, filename) {
  const filesystem = await disk.readFilesystem(archive);
  return await disk.readFile(filesystem, filename, filesystem.getFile(filename));
}
...
module.exports.extractAllAsync = async function (archive, dest) {
  const filesystem = await disk.readFilesystem(archive)
  const filenames = filesystem.listFiles()
  ...
  for (const fullPath of filenames) {
    ...
    if (file.files) {
      ...
      // try to delete output file, because we can't overwrite a link
      try {
        await fsUnlink(destFilename)
      } catch (e) { console.log(e) }
      const linkTo = path.join(relativePath, path.basename(file.link))
      await fsSymlink(linkTo, destFilename)
    } else {
      // it's a file, extract it
      const content = await disk.readFile(filesystem, filename, file)
      await fs.writeFile(destFilename, content);
    }
  }
}
